...
CREATE OR REPLACE FUNCTION fun_recipe_rating(param_recipe_id BIGINT)
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM recipes WHERE recipe_id = param_recipe_id
    ) THEN
        RAISE EXCEPTION 'No recipe with provided ID'
            USING ERRCODE = 'P0001';
    END IF;
    ...
    INTO
        ...
        rating_count
    FROM recipe_ratings
    WHERE recipe_id = param_recipe_id;

    IF rating_count = 0 THEN
        RETURN NULL;
    END IF;

    avg_rating := total_rating::DOUBLE PRECISION / rating_count;
    RETURN avg_rating;
END;
$$;
